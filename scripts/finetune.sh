...
IMG_SIZE=512
ENCODER='mit_b1' 
TRAINSET='DAVIS16'   #Your Finetune datasets
EVASL_VSOD='False'
EPOCH=150
TRAIN_BATCH_SIZE=4
VAL_BATCH_SIZE=1
...
SAVE_ROOT='Your_running_log_save_path'
LOAD_FROM='Your_trained_model_path' #Load the best-performing training checkpoint on the corresponding dataset for fine-tuning
RESTORE_FROM='None' #Path of the checkpoint to restore training from if needed
...
CUDA_VISIBLE_DEVICES=$GPU python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS --master_port $PORT tools/train.py \
	...
	--train_dataset_repeat_list 5 \
	--val_dataset 'YTBOBJ' 'DAVIS16' 'FBMS'\
